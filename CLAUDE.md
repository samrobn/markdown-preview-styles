...
- **Sideloading this extension into a scratch profile needs registration, not just a symlink**: a bare folder/symlink under `--extensions-dir` is marked removed on scan (sharedprocess.log `Marked extension as removed`, recorded in `.obsolete`) and never activates. Write an `extensions.json` entry (`{identifier: {id: "local.markdown-preview-styles"}, version, location: {$mid: 1, path: <abs>, scheme: "file"}, relativeLocation: <folder-name>}`), delete `.obsolete`, relaunch (tested 2026-07-18, vscode 1.129.1).
