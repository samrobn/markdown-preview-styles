...
Unit tests use a stub markdown-it (no real markdown-it dependency) and cover the public `extendMarkdownIt()` surface: frontmatter parsing (numeric IDs stay strings, BOM/whitespace stripped, `[[wiki-link]]` disambiguated from `[inline, array]`, `mps-hide: true` opt-out), value rendering (URLs, wiki-links, dates, HTML escaping), the line-number core rules (1-indexed `data-mps-line`, blank-line placeholder injection), `data-mps-list-depth` tracking on list tokens, callout rewriting, the wikilink target parser (`parseWikilinkTarget`) and resolver (`resolveWikilinkTarget`), index machinery (`addToIndex`/`removeFromIndex`), the `mps_wikilink` inline rule against an injected index, the `mps_block_anchors` core rule, note transclusion paths (resolved hit, embedNotes off, oversized target, cycle cap, image-still-works regression), and the stale-preview mitigation helpers (`markChangedDocument` / `refreshStalePreviewOnTabActivation` / `refreshStalePreviewOnWindowFocus`).
...
| Version bump in `package.json`                    | Rename symlink folder to match; accept VS Code's reload-extensions prompt (full Cmd+Q rarely needed) |
...
**Harness fidelity - mirror VS Code's CSS chrome too, and know what the harness can't show.** Beyond the wrapper, the harness must reproduce VS Code's `markdown.css` chrome that affects geometry: `html, body { padding: 0 26px }` (plus the never-reset 8px default body margin) put the content's left edge ~104px in, not at the gutter's 5em. A viewport-edge cap (`max-width: calc(100vw - Nrem)`) must clear that whole inset **and** the vertical scrollbar - `100vw` includes the scrollbar but the usable width doesn't. **Headless Chromium renders 0-width (overlay) scrollbars**, so the harness can never reproduce scrollbar-induced horizontal overflow directly. Test the invariant instead (left inset + resolved cap + scrollbar allowance ≤ `innerWidth`), and confirm the actual scroll live. When a layout bug "can't be reproduced in the harness", suspect a chrome/scrollbar the harness doesn't model before doubting the report. Two more: computed-style assertions don't prove paint (overflow clipping leaves getComputedStyle intact - screenshot anything visibility-shaped), and mirror CSS vars in the form VS Code emits, not their fallback units (`--markdown-line-height` is unitless; the harness exercising the `22px` fallback green-lit a calc that was type-invalid live). And the live preview stacks three style layers before ours - webview-host defaults (the inline-code chip), markdown.css, and injected `--vscode-*` theme vars; the harness must mirror all three and order them before the inlined style.css, or `var()` fallbacks and cascade wins silently diverge from live (links rendered body-grey while blue live).

### Code-block gutter numbers: the `<pre>` must stay `position: static`

A positioned pre is its own ::before's containing block, so its `overflow: auto` scrollport clips the gutter number - computed styles stay correct while nothing paints (screenshot is the only harness-level proof; render.js pins `position: static` as the discriminator). Static, the number's containing block is `.markdown-body` (positioned for this) or an ancestor `.code-line`, which abspos boxes escape overflow through - preview.js measures the number's left AND top against `offsetParent`, re-aligned by the MutationObserver plus `fonts.ready` and a ResizeObserver (a late image load reflows with no DOM mutation). Two attr shapes feed the mirror: fences carry `code-line`/`data-mps-line` on the inner `<code>`, indented blocks on the `<pre>` itself - hence `.code-line:not(pre)` on the positioning rule and the `static !important` insurance against VS Code's `showEditorSelection` rule.

### `--markdown-line-height` is a unitless multiplier, not a length

VS Code emits the raw `markdown.preview.lineHeight` number (default `1.6`) even though its own markdown.css falls back to `22px`. A bare `var()` added to a length is a type-invalid calc, silently dropped - live only, because the harness's fallback path stays valid. Multiply by `1rem` (markdown.css sets the font-size var on `html`, so rem tracks the setting), and keep the harness setting the var in the unitless form VS Code serves.
...
- **Launch via the app binary, never the `code` CLI**: `"/Applications/Visual Studio Code.app/Contents/MacOS/Electron" --user-data-dir <scratch> --remote-debugging-port=9222 ... &` backgrounded. The CLI goes through the running installation (launch/kill cycles via `code` visibly disturbed the developer's own windows; exact mechanism unpinned) and gives no PID; the direct launch is isolated and yields one via `$!`.
  Handoff trap: if an instance already holds that `--user-data-dir`, the new launch silently hands off to it and exits - the launcher PID dies and the new flags (`--remote-debugging-port`, `--disable-extension`) are ignored. Check `pgrep -f <udd-path>` for a live instance before launching, and verify the launcher PID is still alive after the settle sleep.
- **Shut down by that exact PID, or gracefully over CDP - never `pkill -f <pattern>`**: a pattern kill reached beyond the scratch instance and took out real windows. Killing the launcher PID can leave Electron helper processes holding the profile - kill every PID from `pgrep -f <udd-path>` instead (the udd path is unique to the scratch instance, so this pattern is safe).
...
- **Sideloading into a scratch `--extensions-dir`: a version-suffixed symlink name usually suffices** (bare `local.markdown-preview-styles-0.2.0` -> worktree activated, multiple runs, tested 2026-07-18, vscode 1.129.1). If the scan instead marks it removed (sharedprocess.log `Marked extension as removed`, recorded in `.obsolete` - observed same day; unversioned folder name suspected, cause unpinned): write an `extensions.json` entry (`{identifier: {id: "local.markdown-preview-styles"}, version, location: {$mid: 1, path: <abs>, scheme: "file"}, relativeLocation: <folder-name>}`), delete `.obsolete`, relaunch.
- The installed `~/.vscode/extensions` symlink follows the **main checkout** - a scratch instance without `--extensions-dir` tests whatever that checkout has checked out (a detached spike branch once masqueraded as main). Point a scratch `--extensions-dir` symlink at the worktree to E2E branch code.
- **Driving the workbench over CDP**: open the palette with F1 - Cmd+Shift+P doesn't register (Cmd+\, Cmd+1/2, Cmd+W do). `Input.insertText` lands in the *document* whenever the palette didn't actually open - verify `.quick-input-widget` is visible before typing, or the filter text corrupts the fixture. Quickpick selection by ArrowDown count is non-deterministic (recently-used reordering) - read the `.monaco-list-row` rects and click the row. Mouse-wheel into the preview's OOPIF hangs the CDP call on both page and iframe targets - scroll via `Runtime.evaluate` `scrollBy` on the inner frame.
...
### Rendered diff (SCM side-by-side preview panes)

Each side is a preview webview running our full pipeline; spliced `<span data-diff-start/end>` markers are handled by `mps_diff_markers` + the `mps_frontmatter` strip (rationale in their comments). Two structural facts: **changed regions are wrapped in diff-chunk containers**, so `.markdown-body > X` selectors (wide-content breakout + cap) fire only on unchanged regions - an E2E fixture probing them must keep the probed content in an *unchanged* region or assertions pass vacuously (bit once). And the diff renders its own `<table class="frontmatter">` - suppressed in style.css, exists on no other surface.
...
### The preview restores stale HTML on tab revisit (upstream vscode#147718)

The preview only rewrites `webview.html` - the string a rebuilt webview reloads, carrying the frozen `data-initial-md-content` snapshot the webview script re-injects - on **full renders**: first open, forced refresh, resource change, or a render fired while the tab is hidden. Changes arriving while the tab is **visible** are `updateContent` postMessage DOM patches that never touch it. The preview-as-editor (`vscode.markdown.preview.editor`) registers without `retainContextWhenHidden` (the `retainContextWhenHidden:!0` visible in the bundle belongs to the unrelated WYSIWYG `vscode.markdown.editor`), so hiding the tab destroys the webview; re-showing reloads the stale snapshot, and the version guard - advanced when the patch was *sent* - suppresses any corrective render. Net: edit-while-visible → tab away → back = preview regressed to the last full render, stuck until the next document change or forced refresh. Edits landing while the tab is hidden take the full-render path (`!visible` forces it) and self-heal - which is why the bug looks intermittent. Verified against the 1.127.0 bundle; deterministic repro: preview visible → external edit (patch applies) → switch tab → switch back.

`initStaleRefresh` in `extension.js` mitigates: mark markdown docs on `onDidChangeTextDocument`; when a preview-editor tab for a marked doc becomes active, fire one `markdown.preview.refresh`. The refresh is **global** (every preview re-baselines its `webview.html`), so all marks present at dispatch are serviced together - that's also why the gate matters: unconditional refresh-on-activation would full-reload every preview on every preview-tab click. Marks are deleted only after the refresh resolves (failure keeps them for a retry on the next activation), and only the dispatch-time snapshot is deleted (a mark added mid-refresh survives - its render may predate the change). A second trigger covers the away-from-keyboard case (an external writer edits while the user is away and the tab stays active, so activation never fires; the presumed staleness mechanism - webview destroyed/restored with no tab event while away - is the leading hypothesis from a live recurrence, bundle-verified only for group-visibility restores, not for plain OS focus loss): marks made while the window is unfocused are tracked in a parallel set and serviced when the window regains focus, gated the same way (active preview-editor tab, marked doc). Focused-time marks are deliberately excluded from the focus trigger - the user's own edits patch the live webview, and servicing them would full-reload the preview on every cmd-tab back. Known gaps, accepted for the soak: group-visibility restores **while the window stays focused** (maximising another group, Zen Mode) destroy/restore the webview without firing any tab event (verified in `webviewEditor.ts`: `layout()` → `setEditorVisible` off dimensions alone) - forced refresh remains the manual fix there; and `isActive` is a snapshot, so a metadata change on an already-active marked tab costs one redundant refresh. Classic Open Preview panels (`TabInputWebview`) share the upstream bug but expose no `uri` to gate on - unhandled, we don't use them.

### The static preview snaps the text editor's viewport at click time (upstream vscode#319080)

The static preview (preview-as-editor - what `editorAssociations` opens) does not live-sync the text editor while its preview scrolls. Instead `TopmostLineMonitor` stores the preview's topmost line per document (`setPreviousStaticEditorLine`, written on every preview scroll, **never cleared**) and replays it into a text editor later: deterministically on `onDidChangeActiveTextEditor` (`revealRange AtTop`, no config check at that call site), and via a second click-triggered path that landed up to ~2.3s after the click in scratch testing (mechanism unpinned - only three `Ib`/scrollEditorToLine call sites exist in the bundle, and with no activation change the late reveal must ride the `revealLine`→live-sync path; what makes the webview post late was not traced). Net: in a split (text editor beside a static preview), clicking into the editor teleports the viewport to the preview's stored line, stranding the just-clicked caret off-screen. Reproduced in a scratch profile with zero extensions; A/B/A-verified that `markdown.preview.scrollEditorWithPreview: false` suppresses it - but that kills preview scroll sync wholesale.

`initClickSnapGuard` in `extension.js` counters it: a mouse selection change in a markdown text editor arms a 4s watch that tracks the viewport top per visible-range event; an event that both teleports the top by more than a viewport in one step (real scrolling streams small per-frame steps; observed snaps jump 250+ lines) and strands the clicked caret off-screen is judged to be the snap, and the pre-teleport viewport is restored (`AtTop`). If the snap lands before the selection notification (caret already off-screen when it arrives), the caret is centred instead - the pre-jump viewport is unknowable. Keyboard-driven flows never arm, so the designed "Reopen Editor With Text Editor lands at the preview's position" behaviour is untouched. Known residuals: a same-spot re-click fires no selection event and cannot arm (that jump goes unguarded); a snap later than 4s is not ours to judge; a full-page scrollbar yank within 4s of a click gets countered once. Live-verified over scratch-profile CDP: snap at click+1.1s teleported [531,573]→[793,799], guard restored line 531 within 9ms.

Verifying this class of behaviour needs a **fresh** preview webview: a webview *restored* by window reload or relaunch never posts the scroll lines that prime the upstream store, so the snap silently never fires and any repro run reads as a false pass. Rebuild the preview tab (Reopen Editor With → Markdown Preview) before trusting a negative.
...
Unit tests don't catch this - they assert on parser output before the renderer runs; only render-level tests or the live preview surface it.
...
### The native broken-image fallback is un-spaceable - blank the alt and draw your own

Chromium renders a failed `<img>` as `[glyph][alt]` anonymous internals: `word-spacing` is inert, a spacer `::before` lands before the glyph, and a leading space in the alt renders only when set AFTER the placeholder has painted - set during the error handler it collapses (measured width and pixels disagree). `alt=""` collapses the native fallback entirely; preview.js stashes the name in `data-mps-alt` (+ `aria-label`/`role="img"`) and CSS draws the placeholder - masked icon `::before` + `attr(data-mps-alt)` `::after`. Generated content isn't find-in-page matchable or selectable - accepted trade-off.
...
`markdown-it-wikilinks` exists, supports the pact subset (`name|alias|#heading`), and is what Foam uses. We did not adopt it. Reasons:
...
### Webview-to-extension-host writes: only a REAL click on a REAL vscode:// anchor works

The preview checkbox toggle (`a.mps-task-toggle` in preview.js + the UriHandler in extension.js) is the one channel through which the preview can trigger a source edit. Every alternative was click-tested live and failed:

- **`window.location.href = 'vscode://...'`** navigates the preview document away - blank webview, rebuilt stale on every tab revisit. It is in-place navigation, not a protocol launch, gesture or no gesture.
- **Synthetic `anchor.click()`** does the same, even inside a real click's transient activation - Chromium gates external-protocol launches on the click being *trusted*, not on activation.
- **Hidden `iframe.src`** is CSP-blocked under the preview's default Strict security policy ("Some content has been disabled in this document" banner). Requiring lowered preview security is not shippable.
- **`command:` URIs** are dead in the preview (no `enableCommandUris` on its webview, deliberate), and the webview's message vocabulary (`openLink`, `didClick`, `revealLine`...) is handled only by markdown-language-features' own code - a third-party extension cannot receive webview messages.

What works: preview.js wraps the checkbox in a genuine `<a href="vscode://local.markdown-preview-styles/toggle?doc=...&line=...">`; the user's real click activates it natively (`vscode:` is in the webview click handler's pass-through scheme list), VS Code routes the URI to `registerUriHandler` (one-time per-machine confirm, remembered in `extensionUrlHandler.confirmedExtensions`), and the handler applies a WorkspaceEdit. Routing goes to the *focused window's* extension host - after changing the handler, reload the extension in **every** window (full quit/relaunch) or deep links silently drop in stale windows.

Two MAIO (Markdown All in One) facts the wrap depends on: its `checkbox.css` sets `pointer-events: none` on the input (click-transparent by design) and `margin-left: -20px` (the box hangs in the UL's *padding gutter* - a fall-through click targets the UL, not the li, so hit-testing the li never works). The anchor takes over the negative margin; style.css pins the input inside it. MAIO passes `{enabled: true}` to markdown-it-task-lists, so the inputs are NOT `disabled` (the li carries an `enabled` class) - which is why the input must be kept out of the tab order (`tabIndex -1`): its native Space toggle would set the **dirty-checkedness flag**, the same reason the optimistic flip writes the `checked` ATTRIBUTE, never the property. A property write permanently decouples rendered state from the DOM differ's attribute-level patches.

Two more traps: `handleUri` receives `uri.query` percent-decoded exactly once (undoing preview.js's `encodeURIComponent`) - parse it with the anchored regex in `parseToggleDeepLink`, never `URLSearchParams`, which decodes a second time (corrupts `%` in paths) and applies form-urlencoded semantics (`+` becomes a space). And the anchor wrapper is absent from freshly rendered markup, so VS Code's morphdom-fork differ discards and rebuilds it (locally, content-safe - traced in the bundle) whenever a patch touches that li; the MutationObserver's retrofit pass re-wraps next frame. Renderers using task-lists *label mode* are deliberately skipped - a label's synthetic click would fire the toggle from anywhere on the line.
...
- **Workspace-aware character.** The wikilink upgrade turned a pure markdown-it plugin into a real VS Code extension shape (`require('vscode')`, workspace indexing, `FileSystemWatcher`s, `onDidChangeConfiguration`) - new features can use the VS Code API. But the markdown-it pipeline itself should stay framework-free where possible - `parseWikilinkTarget` and `resolveWikilinkTarget` have no `vscode` dependency, which is what makes them unit-testable in plain Node.

## Files

- `extension.js` - `activate(context)` + `extendMarkdownIt(md)`. Four core rules (`mps_block_anchors`, `mps_callouts`, `mps_blank_lines`, `mps_frontmatter`), two inline rules (`mps_embed`, `mps_wikilink`) and their renderers (`mps_wikilink`, `mps_embed_note`). Workspace `.md` index built on activate, maintained by per-root `FileSystemWatcher`s; `initStaleRefresh` works around the upstream stale-restore bug and `initClickSnapGuard` the upstream click-time viewport snap (see gotchas). Module-level mutable state: `wikiIndex`, `wikiConfig`, `wikiRoots`, `_activeWatchers`, `wikiReadFile`, `wikiStatFile`, `_mdChangedSinceFullRender`, `_clickSnapGuard`. Exports the resolver / parser / index helpers + `__setWikiStateForTest` / `__resetWikiStateForTest` for tests and the visual harness.
- `style.css` - all CSS contributed via `markdown.previewStyles`. Typography block (bundled Martian Mono for headings/gutter/Properties, iA Writer Quattro variable cuts for body prose; woff2s + OFL licences in `fonts/`), line-number gutter, hover-indicator suppression, Properties table, callouts, inline-code tweaks, image embeds (`mps-embed-image` + broken-image placeholder), note transclusion (`mps-embed-note` container + `mps-embed-cycle` style).
- `preview.js` - contributed via `markdown.previewScripts`, runs in the webview. Measures each `.code-line`'s `offsetLeft` and sets `--mps-before-left` so the line-number gutter aligns at every nesting depth. Mirrors the code-block line number (fence and indented attr shapes) onto each `<pre>` (`.mps-pre-line` + `data-mps-line`) because the inner `<code>`'s ::before is clipped by the pre's scroll box - one top-anchored number per code block. Also handles the `attachments/` retry and broken-image styling for image embeds.
- `example.md` - exercises every feature; preview to visually verify changes.
- `test/test.js` - assertions against a stub markdown-it (no real markdown-it dep). Run via `node test/test.js`. 100+ tests; coverage enumerated under Commands.
