...
- Headings render in **Martian Mono Light** (bundled, SIL OFL 1.1 - `fonts/OFL.txt`), a light monospace display treatment: weight 300, a full heading scale (`h1` 40px down to `h6` 12px at the default root), theme-default colour. All the knobs (face, weight, tracking, sizes) are CSS custom properties in the typography block at the top of `style.css`.
